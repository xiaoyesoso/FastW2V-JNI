...
build_for_abi() {
    ...
    # 自动探测 ONNX Runtime Android SDK (如果存在)
    local ORT_SDK_DIR="ort_android_sdk"
    if [ -d "$ORT_SDK_DIR/headers" ]; then
        INCLUDES="$INCLUDES -I$ORT_SDK_DIR/headers"
    fi
    ...
    # 如果有 ORT 库，添加到链接参数
    if [ -f "$ORT_SDK_DIR/jni/$ABI/libonnxruntime.so" ]; then
        LDFLAGS="$LDFLAGS $ORT_SDK_DIR/jni/$ABI/libonnxruntime.so"
        # 复制 ORT 库到 jniLibs 以便打包到 APK
        cp "$ORT_SDK_DIR/jni/$ABI/libonnxruntime.so" "$OUTPUT_DIR/"
    fi
